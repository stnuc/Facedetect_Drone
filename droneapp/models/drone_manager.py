...
class DroneManager(metaclass=Singleton):
    def __init__(self, host_ip='192.168.10.2', host_port=8889,
        ...
        self.pulseCheck = False
        self.sameTime = 123456
        self.count = 0
    ...
    def video_jpeg_generator(self):
        for frame in self.video_binary_generator():
            ...
            if self.pulseCheck == True:
                timeNow = int(time.time())

                #setting face detection
                if (timeNow % 10) == 0:
                    # 이미지 파일 경로
                    image_path_miss = 'C:/Users/minsung/Desktop/Tello_Drone_Han/droneapp/static/img/kms.jpg'
                    image_path_find = 'C:/Users/minsung/Desktop/Tello_Drone_Han/droneapp/static/img/snapshots/snapshot.jpg'

                    # 이미지 파일에서 얼굴 인코딩 생성
                    image_1 = face_recognition.load_image_file(image_path_miss)
                    encoding_1 = face_recognition.face_encodings(image_1)[0]
                    image_2 = face_recognition.load_image_file(image_path_find)
                    encodings_2 = face_recognition.face_encodings(image_2)

                    # 얼굴 유사도 기준 (값은 실험적으로 조정 가능)
                    similarity_threshold = 0.45

                    # image_path_2의 각 얼굴과 image_path_1의 얼굴 비교
                    for encoding_2 in encodings_2:
                        face_distance = face_recognition.face_distance([encoding_1], encoding_2)[0]
                        if face_distance < similarity_threshold:
                            print("이미지 내에서 실종자를 찾을 수 있습니다")
                            
                            backupName = 'C:/Users/minsung/Desktop/Tello_Drone_Han/droneapp/static/img/snapshots/'+'Find'+str(self.count)+'.jpg'
                            findName = 'C:/Users/minsung/Desktop/Tello_Drone_Han/droneapp/static/img/snapshots/Find.jpg'

                            if self.count == 0:
                                os.rename(image_path_find,findName)
                            else:
                                os.rename(findName, backupName)
                                os.rename(image_path_find,findName)
                            self.count += 1
                    
            
                #Snapshot Term setting
                if (timeNow % 1) == 0:
                    if self.sameTime != timeNow:
                        self.is_snapshot = True
                        self.sameTime = timeNow
                        print(self.sameTime,"snapshot check")                     
                    else:
                        if self.is_snapshot == True:
                            time.sleep(0.1)
    ...
    def snapshot(self):
        ...
        if self.pulseCheck == True:
            self.pulseCheck = False
            print("pulseCheck: ", self.pulseCheck)
        else:
            self.pulseCheck = True
            print("pulseCheck: ", self.pulseCheck)
